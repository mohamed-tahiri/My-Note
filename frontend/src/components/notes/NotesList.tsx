  ...
  if (notes.length === 0) {
    return (
      <Box sx={{ textAlign: 'center', py: 10, bgcolor: 'background.paper', borderRadius: 4, border: '1px dashed', borderColor: 'divider' }}>
        <Typography variant="h6" color="text.secondary">Vous n'avez pas encore de notes.</Typography>
        <Typography variant="body2" color="text.disabled">Cliquez sur le bouton + pour commencer.</Typography>
      </Box>
    );
  }
        ...
        return (
          <Grid size={{ xs: 12, md: 4 }} key={p.key}>
            ...
            <Stack direction="row" spacing={1} alignItems="center" sx={{ mb: 2, px: 1 }}>
              ...
              <Typography variant="subtitle1" sx={{ fontWeight: 700, textTransform: 'uppercase', letterSpacing: 1, fontSize: '0.75rem' }}>
                {p.label}
              </Typography>
              <Chip 
