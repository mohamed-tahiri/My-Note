...
import { getById } from '@/api/tasksService'; 
...
export default function TaskDetailPage() {
    ...
    if (!task) return <Typography sx={{ p: 4 }}>Tâche introuvable.</Typography>;
    ...
    const currentStatus = statusConfig[task.status as TaskStatusType] || statusConfig[TaskStatus.PENDING];
    ...
    return (
        ...
        <Box sx={{ borderBottom: '1px solid', borderRadius: '1rem 1rem 0 0', borderColor: 'divider', bgcolor: 'background.paper', mb: 0, px: 4, py: 2 }}>
            ...
            <Stack direction="row" spacing={3} alignItems="center">
                ...
                <Stack direction="row" spacing={1} alignItems="center">
                    ...
                    <Typography variant="caption" sx={{ color: 'primary.main', fontWeight: 700 }}>Créé le {new Date(task.createdAt).toLocaleDateString()}</Typography>
                </Stack>
                </Box>
            </Stack>
        ...
        {/* 2. Content Grid (Utilise toute la page) */}
        <Grid container spacing={0} sx={{ height: 'calc(100vh - 80px)' }}>
            ...
            <Paper elevation={0} sx={{ p: 3, bgcolor: 'background.paper', border: '1px solid', borderColor: 'divider', borderRadius: '16px' }}>
                ...
                {task.description || "Aucune description détaillée n'a été ajoutée à cette tâche."}
                </Typography>
            </Paper>

            {/* Section Historique ou Commentaires (Placeholders pour le design) */}
            <Box sx={{ mt: 6 }}>
                <Typography variant="subtitle2" color="text.secondary" sx={{ fontWeight: 800, mb: 2 }}>ACTIVITÉ</Typography>
                <Typography variant="body2" color="text.disabled" sx={{ fontStyle: 'italic' }}>Bientôt : Suivez l'historique des modifications ici.</Typography>
            </Box>
            </Grid>
            ...
            <Stack spacing={4}>
                ...
                <Stack spacing={1.5}>
                    ...
                    <Paper key={u.id} sx={{ p: 1, borderRadius: '12px', border: '1px solid', borderColor: 'divider', display: 'flex', alignItems: 'center', gap: 2 }}>
                        <Avatar src={u.avatarUrl} sx={{ width: 32, height: 32 }}>{u.firstName?.charAt(0)}</Avatar>
                        <Box>
